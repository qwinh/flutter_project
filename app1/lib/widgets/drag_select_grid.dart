...
//
// Ported from app2 with full attribution.
//
// Usage:
// ```dart
// DragSelectGrid(
...
class DragSelectGrid extends StatefulWidget {
  ...
  /// Called continuously while dragging. Provides [startIndex, endIndex]
  /// (inclusive) that should be selected/deselected.
  final void Function(int startIndex, int endIndex) onSelectionUpdate;

  /// Called when the drag gesture ends.
  final void Function() onSelectionEnd;

  /// Called when an item is tapped (navigation or toggle in selection mode).
...
class _DragSelectGridState extends State<DragSelectGrid> {
  ...
  // Cached cell dimensions (calculated on layout).
  double _cellWidth = 0;
  double _cellHeight = 0;
  final GlobalKey _gridKey = GlobalKey();

  // Auto-scroll speed in logical pixels per timer tick (16 ms).
  ...
  int _indexFromLocalPosition(Offset localPosition) {
    ...
    final adjustedY =
        localPosition.dy + scrollOffset - widget.padding.top;
    ...
    final maxRow =
        ((widget.itemCount - 1) / widget.crossAxisCount).floor();
